# Architecture — Personer

This document describes how Personer is structured: the process model, the security
...
│  - Exposes a typed PersonerApi on window.api                   │
...
└───────────────────────────────────────────────────────────────┘
                │
                ▼
        Encrypted SQLite file (.personerdb)  ── optionally in iCloud Drive
```
...
- **Preload** — the only bridge. It uses `contextBridge.exposeInMainWorld('api', …)` to
  publish a **typed** `PersonerApi`. It forwards a fixed set of channels and nothing else.
...
│  ├─ types.ts             # Task, Note, DatasourceRef, SessionState, IpcResult<T>
│  ├─ ipc.ts               # Centralized IPC channel name constants
│  └─ api.ts               # PersonerApi interface (shape of window.api)
│
├─ main/
...
│  │  ├─ migrate.ts        # idempotent DDL bootstrap + user_version
│  │  └─ repositories.ts   # taskRepo / noteRepo → map rows to domain types
│  ├─ datasource/
│  │  ├─ config.ts         # recent datasources (plaintext userData/personer.config.json)
...
│
├─ preload/
│  ├─ index.ts             # builds PersonerApi from IPC channels, exposes on window.api
...
renderer: session() → { unlocked: false }
   ...
   ▼
LockGate
   ├─ Recent datasources (from userData/personer.config.json — paths + labels only)
